...
function jervisReservationNotFound(error) {
  const message = clean(error?.message);
  return /reservation cannot be found|not found|404/i.test(message);
}

function jervisReservationPropertyMatches(reservation, jervisPropertyId) {
  const expected = clean(jervisPropertyId);
...
async function getExistingJervisReservationForBooking(
  booking,
  jervisPropertyId,
  config = {}
) {
  ...
  try {
    const reservation = await getJervisReservation(bookingId, config);
    if (!reservation || !jervisReservationPropertyMatches(reservation, jervisPropertyId)) {
      return null;
    }
    return reservation;
  } catch (err) {
    if (jervisReservationNotFound(err)) return null;
    throw err;
  }
}

const JERVIS_PINCODE_POLL_DELAYS_MS = [0, 750, 1500, 2500, 4000, 6000];
...
async function fetchJervisDirectAccessCode(booking, config = {}, existing = null) {
  ...
  if (!jervisReservationId) {
    ...
    existingReservation = await getExistingJervisReservationForBooking(
      booking,
      jervisPropertyId,
      config
    );
    jervisReservationId = extractJervisReservationId(existingReservation);
    ...
    await upsertAccessCodeReleaseMetadata({
      ...
      raw: {
        ...
        ...(createResponse
          ...
          : {
              jervisReservationLookupSource: "booking-id",
              jervisReservation: existingReservation,
            }),
      },
    });
  } else {
