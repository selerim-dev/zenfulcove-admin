...
// the reservation exists: Jervis can 400 it for data we don't control (it
// merges guest phones from OTA syncs, and Expedia sends masked numbers that
// fail its "Phone number must be valid" check). The pincode may already be
// provisioned on the reservation, so an update failure must not abort the
// lookup — capture the error and let the pincode poll decide.
