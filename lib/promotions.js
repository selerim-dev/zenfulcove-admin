...
async function resolveRecipients({ selectedLists, logs }) {
  ...
  for (const listKey of selected) {
    const list = definitions[listKey];
    ...
    try {
      ...
      logs.push(
        nowLog(
          `Loaded ${contacts.length} contact(s) from ${list.label} list ${list.listId}`,
          "info"
        )
      );
    } catch (err) {
      logs.push(
        nowLog(
          `Failed to fetch ${list.label} list ${list.listId}: ${err.message}`,
          "failed"
        )
      );
    }
...
function validatePromotionInput({
  ...
  if (shouldSendEmail && !String(subject || "").trim()) {
    logs.push(nowLog("Skipped: email subject is required", "skipped"));
    return false;
  }

  if (shouldSendEmail && !String(emailBody || "").trim()) {
    logs.push(nowLog("Skipped: email message body is required", "skipped"));
    return false;
  }
    ...
    for (const contact of emailRecipients) {
      ...
      try {
        ...
        logs.push(
          nowLog(
            mode === "test"
              ? `[TEST SEND] Routed email originally for ${originalEmail} to ${to}`
              : `Sent email promotion to ${originalEmail}`,
            "success"
          )
        );
      } catch (err) {
