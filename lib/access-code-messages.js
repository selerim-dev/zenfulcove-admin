...
export function buildAccessCodeTemplateData({
  ...
  const text = messageText(data, includeAccessCode);
  return {
    ...data,
...
function releaseFrom(automationConfig) {
  return {
...
export async function sendAccessCodeForBooking({
  ...
  const templateId = clean(config.sendgridTemplateId);
  if (!templateId) return result("skipped", "No SendGrid code template ID configured");

  const context = bookingContext(booking, fallback);
  if (!context.bookingId) return result("skipped", "No booking ID available");
  if (!context.guestEmail) {
    return result("skipped", `Skipped booking ${context.bookingId}: no guest email`);
  }
  ...
  if (dryRun) {
    return result(
      "success",
      `[DRY RUN] Would send access code to ${context.guestEmail} | booking ${context.bookingId} | source ${resolved.source || "unknown"}`,
      { templateData }
    );
  }
  ...
  try {
    await sendTemplateEmail({
      ...
      templateId,
      from: releaseFrom(automationConfig),
      data: templateData,
    });
    ...
    return result(
      "success",
      `Sent access code to ${context.guestEmail} (booking ${context.bookingId})`
    );
  } catch (err) {
    ...
    return result(
      "failed",
      `Failed to send access code for booking ${context.bookingId}: ${detail}`
    );
  }
}
...
export async function sendMissingFormEmailForBooking({
  ...
  const templateId = clean(
    config.missingFormTemplateId ||
      config.noFormTemplateId ||
      config.formRequiredTemplateId
  );
  if (!templateId) {
    return result("skipped", "Form not submitted yet; no missing-form template configured");
  }
  ...
  if (blockedAttemptWasToday(existing, templateId)) {
    return result(
      "skipped",
      `Skipped booking ${context.bookingId}: missing-form template already sent today`
    );
  }
  ...
  if (dryRun) {
    return result(
      "success",
      `[DRY RUN] Would send missing-form code reminder to ${context.guestEmail} | booking ${context.bookingId}`,
      { templateData }
    );
  }
  ...
  try {
    ...
    await markAccessCodeReleaseBlocked({
      ...
      checkinDate: context.checkinDate,
      templateId,
      error: "Form not submitted yet; missing-form template sent.",
      raw: { reason: "missing-form-template" },
    });
    return result(
      "success",
      `Sent missing-form code reminder to ${context.guestEmail} (booking ${context.bookingId})`
    );
  } catch (err) {
    const detail = err.response?.body?.errors?.[0]?.message || err.message;
    await markAccessCodeReleaseBlocked({
      ...
      templateId,
      error: detail,
      raw: { reason: "missing-form-template-failed" },
    }).catch(() => {});
    return result(
      "failed",
      `Failed to send missing-form template for booking ${context.bookingId}: ${detail}`
    );
  }
}
...
export async function maybeSendSameDayAccessCodeForSubmission({
  ...
  if (stayDetails.checkinDate !== todayCentral()) {
    return result(
      "skipped",
      `Submitted stay starts ${stayDetails.checkinDate || "on an unknown date"}, not today`
    );
  }
