...
export async function sendAccessCodeForBooking({
  ...
  if (dryRun) {
    ...
    return result(
      "success",
      `[DRY RUN] Would post Lodgify access-code message to booking ${context.bookingId}`,
      { templateData }
    );
  }
...
export async function sendMissingFormEmailForBooking({
  ...
  if (dryRun) {
    return result(
      "success",
      `[DRY RUN] Would post missing-form Lodgify message to booking ${context.bookingId}`,
      { templateData }
    );
  }
