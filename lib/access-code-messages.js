...
async function sendGuestMessageWithVerifiedDelivery({
  ...
  if (useEmailFallback && !threadGuid) {
  ...
  const verification = await verifyLodgifyMessageDelivery({
    booking,
    context,
    subject,
    message: preparedMessage.message,
  ...
  if (routeFallbackNeeded || useEmailFallback) {
    const expectedRoute =
      ...
      if (!routeFallbackNeeded) {
        return {
          ...
          details: {
            ...
            ...fallback.details,
            lodgifyPosted: true,
            emailFallbackSkipped: "missing-email",
          },
        };
      }
      throw new Error(
    ...
    return {
      deliveryChannel: "sendgrid",
      deliveryFallback: routeFallbackNeeded
        ? "lodgify-unrouted-email"
        : "lodgify-channel-email-backup",
      details: {
        ...details,
        ...fallback.details,
        lodgifyPosted: true,
        lodgifyFallbackReason:
          verification.lodgifyDeliveryIssue ||
          (routeFallbackNeeded
            ? "missing-channel-route"
            : "configured-channel-email-backup"),
      },
    };
  }
...
function deliveredAction(delivery, { lodgify, email, unrouted }) {
  ...
  if (delivery?.deliveryFallback === "lodgify-channel-email-backup") {
    return `${lodgify}; sent guest email fallback backup`;
  }
