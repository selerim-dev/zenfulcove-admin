...
export async function sendAccessCodeForBooking({
  ...
  const useEmailFallback = shouldUseEmailFallback(context, config);
  const deliveryChannel = useEmailFallback ? "sendgrid" : "lodgify";
  ...
  try {
    if (useEmailFallback) {
      await sendEmailFallbackMessage({
        ...
        text: `${messagePrefix || ""}${templateData.lodgifyMessageText || ""}`,
        messageName,
      });
    } else {
    ...
    if (persistState !== false) {
      await markAccessCodeReleaseSent({
        bookingId: context.bookingId,
        templateId: useEmailFallback ? `sendgrid:plain-${decision}` : templateId,
    ...
    return result(
      "success",
      useEmailFallback
        ? `Sent ${messageName} by guest email fallback for booking ${context.bookingId}${bookingSourceSuffix(context)}`
        : `Posted ${messageName} to Lodgify booking thread ${context.bookingId}${bookingSourceSuffix(context)}`,
      {
        bookingId: context.bookingId,
        deliveryChannel,
        ...(useEmailFallback ? { deliveryFallback: "lodgify-channel-email" } : {}),
...
export async function sendCheckinInfoForBooking({
  ...
  try {
    ...
    return result(
      "success",
      useEmailFallback
        ? `Sent check-in information by guest email fallback for booking ${context.bookingId}${bookingSourceSuffix(context)}`
        : `Posted no-code check-in information to Lodgify booking thread ${context.bookingId}${bookingSourceSuffix(context)}`,
      {
        bookingId: context.bookingId,
        deliveryChannel,
        ...(useEmailFallback ? { deliveryFallback: "lodgify-channel-email" } : {}),
...
export async function sendMissingFormEmailForBooking({
  ...
  try {
    ...
    return result(
      "success",
      useEmailFallback
        ? `Sent missing-form reminder by guest email fallback for booking ${context.bookingId}${bookingSourceSuffix(context)}`
        : `Posted missing-form reminder to Lodgify booking thread ${context.bookingId}${bookingSourceSuffix(context)}`,
      {
        bookingId: context.bookingId,
        deliveryChannel,
        ...(useEmailFallback ? { deliveryFallback: "lodgify-channel-email" } : {}),
...
export async function sendWaiverReminderForBooking({
  ...
  try {
    ...
    return result(
      "success",
      useEmailFallback
        ? `Sent waiver reminder "${label}" by guest email fallback for booking ${context.bookingId}${bookingSourceSuffix(context)}`
        : `Posted Lodgify waiver reminder "${label}" to booking thread ${context.bookingId}${bookingSourceSuffix(context)}`,
      {
        bookingId: context.bookingId,
        deliveryChannel,
        ...(useEmailFallback ? { deliveryFallback: "lodgify-channel-email" } : {}),
