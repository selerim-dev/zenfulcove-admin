...
const DEFAULT_LODGIFY_MASKED_LINK_SOURCES = ["BookingCom"];
...
export async function sendAccessCodeForBooking({
  ...
  try {
    ...
    return result(
      "success",
      `Posted ${resolvedMessageKind === "code-only" ? "day-of code" : "access code"} to Lodgify booking thread ${context.bookingId}${bookingSourceSuffix(context)}`,
      {
        bookingId: context.bookingId,
        deliveryChannel: "lodgify",
        decision: resolvedMessageKind === "code-only" ? "code-only" : "access-code",
