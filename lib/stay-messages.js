...
function guestPortalRelayBody(body) {
  const text = clean(body);
  const oldRelay = text.match(/^Guest portal message from [^\n:]+:\s*\n+([\s\S]*)$/i);
  if (oldRelay) return clean(oldRelay[1]);

  if (/^Guest portal submission for this booking\.?/i.test(text)) {
    const newRelay = text.match(/\nMessage:\s*\n+([\s\S]*)$/i);
    return newRelay ? clean(newRelay[1]) : "";
  }
...
export function normalizeStayMessage(message = {}, { source = "local", bookingId = "" } = {}) {
  ...
  const relayBody = message.isGuestPortalRelay ? rawBody : guestPortalRelayBody(rawBody);
  const body = relayBody || rawBody;
  ...
  const direction = relayBody ? "guest" : clean(message.direction) || inferDirection(message);
  ...
  return {
    ...
    attachments,
    isGuestPortalRelay: Boolean(message.isGuestPortalRelay || relayBody),
  };
}
...
export function mergeStayMessages(...groups) {
  const normalizedMessages = groups
    .flat()
    .filter(Boolean)
    .map((message) =>
      normalizeStayMessage(message, {
        source: message.source || "local",
        bookingId: message.bookingId,
      })
    );
  const localGuestBodies = new Set(
    normalizedMessages
      .filter((message) => message.source === "guest-portal" && message.direction === "guest")
      .map((message) => clean(message.body).toLowerCase())
      .filter(Boolean)
  );
  const byKey = new Map();
  for (const normalized of normalizedMessages) {
    if (
      normalized.source === "lodgify" &&
      normalized.isGuestPortalRelay &&
      localGuestBodies.has(clean(normalized.body).toLowerCase())
    ) {
      continue;
    }
    const key = [
