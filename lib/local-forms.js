...
const LOCAL_FORM_SLUG_ALIASES = {
  "welcome-to-zenfulcove": ["guest-info"],
  "guest-info": ["welcome-to-zenfulcove"],
};
...
export function localFormSlugCandidates(slug) {
  const normalizedSlug = String(slug || "").trim();
  if (!normalizedSlug) return [];
  return Array.from(
    new Set([normalizedSlug, ...(LOCAL_FORM_SLUG_ALIASES[normalizedSlug] || [])])
  );
}

export async function getLocalFormBySlug(slug) {
  const candidates = localFormSlugCandidates(slug);
  ...
  if (error) {
    throw new Error(`Failed to load local form ${candidates[0]}: ${error.message}`);
  }

  const forms = Array.isArray(data) ? data : [];
  for (const candidate of candidates) {
    const match = forms.find((form) => String(form.slug || "").trim() === candidate);
...
export async function listLocalFormSubmissions({
  ...
  const slugs = Array.from(
    new Set(
      (formSlugs || [])
        .flatMap((slug) => localFormSlugCandidates(slug))
 ...
 * @param {{ formSlug?: string; bookingId?: string; limit?: number }} options
 */
export async function listLocalFormSubmissionsForBooking({
  formSlug,
  bookingId,
  limit = 10000,
} = {}) {
  const normalizedFormSlug = String(formSlug || "").trim();
  const normalizedBookingId = String(bookingId || "").trim();
  if (!normalizedFormSlug || !normalizedBookingId) return [];
  ...
  if (error) {
    throw new Error(`Failed to load local form submissions for booking: ${error.message}`);
  }
